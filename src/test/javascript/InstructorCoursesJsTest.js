...
QUnit.test('isCourseIDValidChars(courseId)', function(assert) {
    // valid cases
    assert.equal(isCourseIDValidChars('_$course1.2-3'), true, 'Course Id valid');

    // invalid cases
    assert.equal(isCourseIDValidChars('CS10@@'), false, 'Course Id has @ character');
    assert.equal(isCourseIDValidChars('CS100!'), false, 'Course Id has ! character');
    assert.equal(isCourseIDValidChars('CS100#'), false, 'Course Id has # character');
    assert.equal(isCourseIDValidChars('CS100%'), false, 'Course Id has % character');
    assert.equal(isCourseIDValidChars('CS100^'), false, 'Course Id has ^ character');
    assert.equal(isCourseIDValidChars('CS100&'), false, 'Course Id has & character');
...
QUnit.test('getCourseIdInvalidityInfo(courseId)', function(assert) {
    ...
    // invalid cases
    assert.equal(getCourseIdInvalidityInfo(''), DISPLAY_COURSE_COURSE_ID_EMPTY + '<br>', 'Course Id empty');
    assert.equal(getCourseIdInvalidityInfo('   '), DISPLAY_COURSE_COURSE_ID_EMPTY + '<br>',
                 'Course Id only white spaces');
    assert.equal(getCourseIdInvalidityInfo('course*1'), DISPLAY_COURSE_INVALID_ID + '<br>',
                 'Course Id only white spaces');
    assert.equal(getCourseIdInvalidityInfo(generateRandomString(COURSE_ID_MAX_LENGTH + 1)),
                 DISPLAY_COURSE_LONG_ID + '<br>', 'Course Id too long');
});
...
QUnit.test('getCourseNameInvalidityInfo(courseId)', function(assert) {
    ...
    // invalid cases
    assert.equal(getCourseNameInvalidityInfo('   '), DISPLAY_COURSE_COURSE_NAME_EMPTY + '<br>', 'Course name empty');
    assert.equal(getCourseNameInvalidityInfo(generateRandomString(COURSE_NAME_MAX_LENGTH + 1)),
                 DISPLAY_COURSE_LONG_NAME + '<br>', 'Course name too long');
});
...
QUnit.test('getCourseTimeZoneInvalidityInfo(courseTimeZone)', function(assert) {
    ...
    // invalid case
    assert.equal(getCourseTimeZoneInvalidityInfo('InvalidTimeZone'), DISPLAY_COURSE_INVALID_TIME_ZONE + '<br>',
                 'Course time zone invalid');
});

QUnit.test('checkAddCourseParam(courseID, courseName)', function(assert) {

    assert.equal(checkAddCourseParam('valid.course.id', 'Software Engineering'), '', 'All valid values');

    assert.equal(checkAddCourseParam('', 'Software Engineering'), DISPLAY_COURSE_COURSE_ID_EMPTY + '<br>',
                 'Course ID empty');
    ...
    assert.equal(checkAddCourseParam('valid.course-id', 'Software Engineering', 'InvalidTimeZone'),
                 DISPLAY_COURSE_INVALID_TIME_ZONE + '<br>', 'Course time zone invalid');
    ...
    assert.equal(checkAddCourseParam('invalid course id',
                                     generateRandomString(COURSE_NAME_MAX_LENGTH + 1), 'googid|Instructor1|'),
